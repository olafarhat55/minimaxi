...
const normalizeWorkOrder = (d: any) => {
  ...
  const normalize = (wo: any) => {
    ...
    return {
      ...
      action_taken:     wo.action_taken     ?? wo.actionTaken     ?? null,
      root_cause:       wo.root_cause       ?? wo.rootCause       ?? null,
      hours_spent:      wo.hours_spent      ?? wo.hoursSpent      ?? null,
      minutes_spent:    wo.minutes_spent    ?? wo.minutesSpent    ?? null,
      additional_notes: wo.additional_notes ?? wo.additionalNotes ?? null,
      spare_parts:      wo.spare_parts      ?? wo.spareParts      ?? [],
...
const realApi = {
    ...
    .then((data: any) =>
      (Array.isArray(data) ? data : []).map((item: any) => {
        ...
        return {
          ...
          sensorName:   item.sensorName ?? sensorMatch?.[1] ?? null,
 ...
 deleteWorkOrder: (id: string | number) =>
  ...
  getWorkOrderNotes: (id: string | number) =>
  axiosInstance.get(`/work-orders/${id}/notes`),
